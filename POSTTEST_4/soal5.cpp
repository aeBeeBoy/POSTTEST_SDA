#include <iostream>
using namespace std;

struct Node {
    int data;
    Node* next;
    Node* prev;
};

/*
 * Fungsi ini menerima referensi ke pointer head dan tail.
 * Pointer head dan tail akan di-update setelah penukaran.
 */
void exchangeHeadAndTail(Node *&head_ref) {
    // Jika list kosong atau hanya 1 node → tidak perlu ditukar
    if (head_ref == nullptr || head_ref->next == head_ref)
        return;

    Node* head = head_ref;
    Node* tail = head_ref->prev;

    // Jika hanya ada dua node, cukup tukar head dan tail
    if (head->next == tail) {
        head->next = head;
        head->prev = tail;
        tail->next = tail;
        tail->prev = head;
        head_ref = tail;
        return;
    }

    // Simpan node tetangga head dan tail
    Node* headNext = head->next;
    Node* tailPrev = tail->prev;

    // Putus koneksi head dan tail dari posisi lama, lalu tukar
    // Sambungkan tail ke posisi head
    tail->next = headNext;
    tail->prev = tailPrev;
    headNext->prev = tail;
    tailPrev->next = tail;

    // Sambungkan head ke posisi tail
    head->next = tail;
    head->prev = tailPrev;
    tailPrev->next = head;
    tail->prev = head;

    // Update pointer circular
    tail->next->prev = tail;
    head->prev->next = head;

    // Update head_ref menjadi node baru (tail lama)
    head_ref = tail;
}

void printList(Node *head_ref)
{
    if (head_ref == nullptr)
    {
        cout << "List kosong" << endl;
        return;
    }

    Node *current = head_ref;
    do
    {
        cout << current->data << " ";
        current = current->next;
    } while (current != head_ref);
    cout << endl;
}

void insertEnd(Node *&head_ref, int data)
{
    Node *newNode = new Node{data, nullptr, nullptr};

    if (head_ref == nullptr)
    {
        newNode->next = newNode;
        newNode->prev = newNode;
        head_ref = newNode;
        return;
    }

    Node *tail = head_ref->prev;
    newNode->next = head_ref;
    newNode->prev = tail;
    head_ref->prev = newNode;
    tail->next = newNode;
}

int main()
{
    Node *head = nullptr;

    // Buat list: 1 <-> 2 <-> 3 <-> 4 <-> 5
    insertEnd(head, 1);
    insertEnd(head, 2);
    insertEnd(head, 3);
    insertEnd(head, 4);
    insertEnd(head, 5);

    cout << "List sebelum exchange: ";
    printList(head);

    exchangeHeadAndTail(head);

    cout << "List setelah exchange head dan tail: ";
    // Expected output: 5 2 3 4 1
    printList(head);

    return 0;
}
